...
//   1. Removed fake NOTIFS array entirely
//   2. Notification panel now shows "No notifications yet" (real placeholder)
//   3. Removed red unread dot from bell icon (no fake unread count)
//   4. Fixed broken JSX from previous commented-out tags
//   5. Issue Map link stays commented out (already done)
//   6. logout now calls clearAuth from context to clear token properly
...
export default function Navbar({ currentPage, setCurrentPage }) {
  const { isLoggedIn, user, clearAuth, showToast } = useApp()
  ...
  const links = [
    ...
    // { id:'map', label:'Issue Map' },  ← hidden for now, add back later
  ]
  ...
  return (
    <>
      ...
      <nav style={{
        ...
        <div style={{
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, marginLeft:16, flexShrink:0 }}>
            ...
            {/* Bell icon — no red dot since no real notifications yet */}
            <div style={{ position:'relative', flexShrink:0 }}>
              <button
                onClick={() => setNotifOpen(prev => !prev)}
                aria-label="Notifications"
                style={{ width:36, height:36, background:'#f9fafb', border:'1px solid #e5e7eb',
            ...
            {isLoggedIn ? (
              <>
                {/* Real user initials instead of hardcoded "RJ" */}
      ...
      {/* ── Notification panel — no fake data ── */}
      {notifOpen && (
        <div
